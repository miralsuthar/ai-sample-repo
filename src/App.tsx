...
import { ColorContext, ColorContextProvider, TColorContext } from './components/ColorContext';
...
function App() {
  ...
  const { setPrimary300 } = useContext(ColorContext) as TColorContext;

  useEffect(() => {
    console.log("shades: ", shades[5])
    setPrimary300(shades[3]);
  }, [shades])


  return (
    <ColorContextProvider>
      <main className="min-h-screen max-w-screen py-[7.56rem] px-[7.43rem] flex flex-col gap-4 justify-center items-center" style={{
        background: `${primaryColor}`
      }}>
        <section className='flex justify-between w-full items-center'>
          <div className='bg-[#26252A] p-4 w-max rounded-full'>
            <HuePicker className='rounded-full overflow-hidden h-4' color={primaryColor} onChange={(v) => setPrimaryColor(v.hex)} />
          </div>
          <div className='flex gap-[0.315rem] bg-[#26252A] p-4 rounded-full w-max'>
            {shades?.map((color, i) => (
              <div style={{ backgroundColor: color, }} className='h-4 w-20 rounded-full' key={i}></div>
            ))}
          </div>
        </section>
        <section className='w-full p-4 bg-[#17171A] h-[54.3125rem] rounded-[1.5rem] relative'>
          <PanelGroup direction='vertical'>
            <Panel defaultSize={60}>
              <PanelGroup direction='horizontal'>
                ...
                <PanelResizeHandle className="w-2 h-10 my-auto bg-white" />
                <Panel minSize={50} defaultSize={60}>
                  <div className='w-full h-full bg-[#26252A] rounded-xl p-1'>
                    <FilterBar checkBoxColor={shades[3]} shades300={shades[2]} />
                  </div>
                </Panel>
                <PanelResizeHandle className="w-2 h-10 my-auto bg-white" />
                <Panel minSize={20} defaultSize={20}>
                  <div className='w-full h-full bg-[#26252A] rounded-xl'>
                    <DraggableClock shades100={shades[0]} shades200={shades[1]} />
                  </div>
                </Panel>
              </PanelGroup>
            </Panel>
            <PanelResizeHandle className="h-2 w-4 mx-auto bg-white" />
            <Panel defaultSize={20}>
              <div className='w-full flex h-full p-8 bg-[#26252A] rounded-xl'>
                ...
                <div className='ml-auto'>
                  <AnimatedDropdown />
                  {isModalOpen && <Modal shades300={shades[2]} shades400={shades[3]} shades500={shades[4]} shades600={shades[5]} shades700={shades[6]} className='absolute bottom-24 right-5' />}
                  <button onClick={() => setIsModalOpen(prev => !prev)} className='w-16 h-16 flex justify-center absolute bottom-5 right-5 items-center shadow-[3px_6px_10px_0_rgba(0,0,0,0.15)] rounded-full' style={{
